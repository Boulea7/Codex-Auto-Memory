...
- shared/team memory 这轮已经推进为可用的 `Reviewer+Recall` 能力：仓库内 `TEAM_MEMORY.md` + `team-memory/*.md` 会被编成 project-scoped read-only sidecar，并通过 `dreamSidecar.teamMemory`、`resumeContext.suggestedTeamEntries`、`querySurfacing.suggestedTeamEntries` 暴露 reviewer 提示，但不会直接进入 canonical durable memory。
