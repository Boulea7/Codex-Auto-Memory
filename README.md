...
| `cam dream build` / `cam dream inspect` | 构建并审查最小可用 `dream sidecar`；它会把 continuity compaction、query-time relevant refs、`teamMemory` 摘要和 pending `promotionCandidates` 写进可审计的 JSON sidecar，但不会直接改 `MEMORY.md` 或 topic files；后续公开的 `cam dream candidates` / `cam dream review` / `cam dream adopt` / `cam dream promote-prep` / `cam dream promote` 仍然走显式 review lane：durable-memory candidate 只能在显式 promote 后通过现有 reviewer/audit 链路写入 canonical memory，而 instruction-like candidate 继续保持 `proposal-only` |
...
- dream reviewer lane 现在继续收口为 `cam dream candidates` / `cam dream review` / `cam dream adopt` / `cam dream promote-prep` / `cam dream promote` 这组公开面；其中 blocked subagent candidate 需要先 `adopt`，durable-memory candidate 的 `promote` 会走现有 reviewer/audit 链路显式落入 canonical memory，而 instruction-like candidate 继续保持 `proposal-only`，不会直接写 instruction files，只会返回结构化 proposal artifact。
