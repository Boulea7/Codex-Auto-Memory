...
当前最稳的自动记忆主路径仍然是 wrapper：启动时会按 `continuity -> instruction files -> dream refs -> top durable refs -> team/shared refs` 叠层组织 startup 提示；会话结束后，`cam` 会自动从 Codex rollout 日志里提取信息并写入 memory 文件。
...
| 命令 | 作用 |
| :-- | :-- |
| `cam run` / `cam exec` / `cam resume` | 编译 startup memory 并通过 wrapper 启动 Codex；wrapper startup 叠层顺序保持为 `continuity -> instruction files -> dream refs -> top durable refs -> team/shared refs` |
...
| `cam dream build` / `cam dream inspect` | 构建并审查最小可用 `dream sidecar`：`build` 会把 continuity compaction、query-time relevant refs、`teamMemory` 摘要和 pending `promotionCandidates` 写进可审计的 JSON sidecar，而 `inspect` 只读取最新 snapshot / audit / queue 元数据，不会隐式重建 sidecar，也不会直接改 `MEMORY.md` 或 topic files；公开的 dream reviewer lane 现在统一写为 `cam dream candidates` / `cam dream review` / `cam dream adopt` / `cam dream proposal` / `cam dream promote-prep` / `cam dream promote` / `cam dream apply-prep` / `cam dream verify-apply`：其中 blocked subagent candidate 需要先 `adopt`，durable-memory candidate 只能在显式 `promote` 后通过现有 reviewer/audit 链路写入 canonical memory，而 instruction-like 的 `proposal` / `promote` / `promote-prep` / `apply-prep` 都继续保持 `proposal-only`，只返回带有 `patchPreview`、`artifactPath`、`manualWorkflow`、`applyReadiness` 的 proposal artifact；instruction-like lane 也支持在 `promote-prep` / `promote` 阶段显式 `--target-file` override，`promote` 之后会进入 `manual-apply-pending` reviewer 状态，再通过 `verify-apply` 收口，仍然永不自动改 instruction files |
...
| `cam recall search` / `timeline` / `details` | 以 `search -> timeline -> details` 的 progressive disclosure 检索 durable memory；`search` 默认采用 `state=auto`、`limit=8`，先查 active，未命中再回退 archived，且保持只读 retrieval；多词查询现在会跨 `id/topic/summary/details` 聚合命中，而不是要求所有 term 落在同一个字段；JSON 输出现在还会额外暴露 `retrievalMode`、`finalRetrievalMode`、`retrievalFallbackReason`、`stateResolution`、`executionSummary`、`searchOrder`、`totalMatchedCount`、`returnedCount`、`globalLimitApplied`、`truncatedCount`、`resultWindow`、`globalRank`，以及 `diagnostics.checkedPaths[].returnedCount` / `droppedCount`，把 auto-state、global sort、fallback 与 post-limit 行为说清楚；同时还会通过 additive `querySurfacing` 给出 `suggestedDreamRefs`、`suggestedInstructionFiles` 与 read-only `suggestedTeamEntries`，作为 query-time reviewer hints，而不是直接改动 search 结果或 canonical memory；其中 `finalRetrievalMode` 只是对最终结果面的显式别名，`retrievalMode` 继续保留兼容语义 |
...
- `cam session status --json` / `cam session load --json` 现在会额外暴露 `resumeContext`，其中 `suggestedDurableRefs` 与 `suggestedTeamEntries` 都只作为 additive resume hints；它不会替代 shared/local continuity，也不会把 dream candidate 直接升级成 durable memory。
- wrapper startup 的公开口径现在统一为 `continuity -> instruction files -> dream refs -> top durable refs -> team/shared refs`；其中 team/shared refs 仍然是 read-only、non-canonical 的 reviewer hints，而不是第二真相层。
- `cam recall search --json` 现在会额外暴露 `querySurfacing`，其中 `suggestedDreamRefs` / `suggestedInstructionFiles` / `suggestedTeamEntries` 只作为检索时的 reviewer hints；它们不会改变 `results[]` 的排序与过滤，更不会直接触发 promote。
- dream reviewer lane 现在继续收口为 `cam dream candidates` / `cam dream review` / `cam dream adopt` / `cam dream proposal` / `cam dream promote-prep` / `cam dream promote` / `cam dream apply-prep` / `cam dream verify-apply` 这组公开面；其中 blocked subagent candidate 默认先 blocked，必须先 `adopt`；durable-memory candidate 的 `promote` 会走现有 reviewer/audit 链路显式落入 canonical memory，而 instruction-like candidate 的 `proposal`、`promote`、`promote-prep`、`apply-prep` 继续保持 `proposal-only`，不会直接写 instruction files，只会返回带有 `patchPreview`、`artifactPath`、`manualWorkflow`、`applyReadiness` 的 proposal artifact；`verify-apply` 只负责确认人工落地已经完成并把 reviewer 状态收口。
- shared/team memory 这轮已经推进为可用的 `Reviewer+Recall` 能力：`TEAM_MEMORY.md` 现在明确作为 repo-root manifest / index-only 入口，真实可检索 entry 只来自 `team-memory/*.md`；它们会被编成 project-scoped read-only sidecar，并通过 `dreamSidecar.teamMemory`、`resumeContext.suggestedTeamEntries`、`querySurfacing.suggestedTeamEntries` 暴露 reviewer 提示，但不会直接进入 canonical durable memory。
- `dreamSidecarAutoBuild` 现在只在 wrapper startup 这类允许写 sidecar 的运行路径里按需重建 snapshot；`cam memory`、`cam dream inspect`、`cam recall`、MCP retrieval、`cam session status/load` 这些 inspect / retrieval surface 继续保持只读，sidecar 或 team index 缺失时只返回 diagnostics 与 next steps，不隐式写盘。
- release-facing 验证仍要求串行跑 `pnpm test:dist-cli-smoke` 与 `pnpm test:tarball-install-smoke`，避免 `prepack -> rimraf dist` 造成假阴性。
