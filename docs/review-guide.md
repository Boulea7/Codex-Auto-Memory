...
- If durable sync partially succeeds, can reviewers see the pending sync recovery marker without confusing it with the normal sync audit stream?
...
- If continuity Markdown was written but audit append failed, can reviewers see the pending continuity recovery marker in both JSON and text surfaces?
