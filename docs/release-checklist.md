...
- Run `node dist/cli.js hooks install --cwd <path>` from another working directory and confirm the generated hook helper bundle pins `memory-recall.sh` and `post-work-memory-review.sh` to the targeted project root instead of the caller shell cwd.
