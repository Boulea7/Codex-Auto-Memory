...
- 2026-04-14: 继续吸收近几个已合并 PR 的有效 review：`test/recall-command.test.ts` / `test/session-command.test.ts` 现在统一用 `restoreOptionalEnv` 还原可选环境变量，避免把缺失的 `CAM_CODEX_SESSIONS_DIR` 恢复成字符串 `"undefined"`；`src/lib/commands/session.ts` 把只读/可写 runtime 选择收拢到共享 helper；`test/dist-cli-smoke.test.ts` 补上 release-facing reviewer smoke，锁住“shared scope 里仍然 `manual-apply-pending` 的 instruction proposal 不会被后续 project-local rejected artifact 挤掉”的过滤契约。
...
- 2026-04-14: `team-memory` retrieval index 的 stale 判定补上“新增 topic 文件”场景；当旧索引建立后再出现新的 `team-memory/*.md`，`cam recall` 的 read-only surfacing 现在会继续 fail-closed，而不是继续沿用漏掉新文件的旧 index，并由 `test/recall-command.test.ts` 锁住回归。
