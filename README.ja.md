...
| `cam dream build` / `cam dream inspect` | 最小構成の `dream sidecar` を構築・点検する。continuity compaction、query-time relevant refs、保留中の `promotionCandidates` を監査可能な JSON sidecar に保存するが、`MEMORY.md` や topic files は直接変更しない。公開 `cam dream candidates` / `cam dream review` / `cam dream promote` も明示的な reviewer lane を保ち、durable-memory candidate は既存の reviewer/audit 書き込み経路でのみ promote され、instruction-like candidate は引き続き `proposal-only` にとどまる |
...
| `cam session load` / `status` | continuity reviewer surface を確認する。`status --json` / `load --json` では additive `resumeContext` も返り、現在の goal、`suggestedDurableRefs`、instruction files を次の会話へ引き継ぐ reviewer hint として見られる |
...
- `cam session status --json` / `cam session load --json` は additive `resumeContext` も返すようになり、`suggestedDurableRefs` は resume hint として残るが、自動で durable memory へ昇格はしない。
- `cam recall search --json` は additive `querySurfacing` も返すようになり、`suggestedDreamRefs` / `suggestedInstructionFiles` は query-time reviewer hints として見せるだけで、検索結果や promote を自動変更しない。
- dream reviewer lane は `cam dream candidates` / `cam dream review` / `cam dream promote` に収束させているが、durable-memory candidate の promote は既存の reviewer/audit 経路を通じてのみ canonical memory に書き込まれ、instruction-like candidate は `proposal-only` のままで instruction files を直接書き換えない。
