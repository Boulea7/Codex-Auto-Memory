...
| コマンド | 役割 |
| :-- | :-- |
| `cam run` / `cam exec` / `cam resume` | startup memory を生成して wrapper 経由で Codex を起動する。wrapper startup の層順は `continuity -> instruction files -> dream refs -> top durable refs -> team/shared refs` を維持する |
...
| `cam dream build` / `cam dream inspect` | 最小構成の `dream sidecar` を構築・点検する。`build` は監査可能な JSON snapshot / audit / candidate state を書き込み、`inspect` は最新の snapshot・audit・queue metadata だけを読み取る。implicit に sidecar を再構築したり、`MEMORY.md` や topic files を直接変更したりはしない。公開 dream reviewer lane は `cam dream candidates` / `cam dream review` / `cam dream adopt` / `cam dream proposal` / `cam dream promote-prep` / `cam dream promote` / `cam dream apply-prep` / `cam dream verify-apply` に揃える。blocked な subagent candidate は先に `adopt` が必要で、durable-memory candidate は既存の reviewer/audit 経路を通る明示的な `promote` でのみ書き込まれる。instruction-like の `proposal` / `promote` / `promote-prep` / `apply-prep` はすべて `proposal-only` のままで、`patchPreview`、`artifactPath`、`manualWorkflow`、`applyReadiness` を含む proposal artifact を返すだけで instruction files は直接編集しない。instruction-like lane は既定の ranking を変えずに `promote-prep` / `promote` でだけ明示的な `--target-file` override を受け付け、proposal-only promote 後は `manual-apply-pending` reviewer state に入り、`verify-apply` で収束する |
...
| `cam session load` / `status` | continuity reviewer surface を確認する。`status --json` / `load --json` では additive `resumeContext` も返り、現在の goal、`suggestedDurableRefs`、instruction files、read-only な `suggestedTeamEntries` を次の会話へ引き継ぐ reviewer hint として見られる |
...
- `cam session status --json` / `cam session load --json` は additive `resumeContext` も返すようになり、`suggestedDurableRefs` と `suggestedTeamEntries` は resume hint として残るが、自動で durable memory へ昇格はしない。
- wrapper startup の公開 wording は `continuity -> instruction files -> dream refs -> top durable refs -> team/shared refs` に揃える。
- `cam recall search --json` は additive `querySurfacing` も返すようになり、`suggestedDreamRefs` / `suggestedInstructionFiles` / `suggestedTeamEntries` は query-time reviewer hints として見せるだけで、検索結果や promote を自動変更しない。
- dream reviewer lane は `cam dream candidates` / `cam dream review` / `cam dream adopt` / `cam dream proposal` / `cam dream promote-prep` / `cam dream promote` / `cam dream apply-prep` / `cam dream verify-apply` に収束させている。subagent candidate は blocked で始まり、durable-memory candidate の promote は既存の reviewer/audit 経路を通じてのみ canonical memory に書き込まれる。instruction-like の `proposal` / `promote` / `promote-prep` / `apply-prep` は `proposal-only` のままで、`patchPreview`、`artifactPath`、`manualWorkflow`、`applyReadiness` を含む proposal artifact を返すだけで instruction files を直接書き換えない。`verify-apply` は手動適用の確認と reviewer closeout だけを担当する。
- `TEAM_MEMORY.md` は shared team pack の root manifest / index-only 入口として扱い、reviewer-visible team entry は `team-memory/*.md` からのみ作る。`dreamSidecarAutoBuild=true` でも inspect / retrieval surfaces は read-only を保ち、sidecar の再構築は wrapper startup のみが担う。
...
- release-facing 検証では `pnpm test:dist-cli-smoke` と `pnpm test:tarball-install-smoke` を直列で実行し、`prepack -> rimraf dist` による偽陰性を避ける必要がある。
