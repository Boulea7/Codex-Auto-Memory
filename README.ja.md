...
| `cam dream build` / `cam dream inspect` | 最小構成の `dream sidecar` を構築・点検する。continuity compaction、query-time relevant refs、read-only な `teamMemory` hints、保留中の `promotionCandidates` を監査可能な JSON sidecar に保存するが、`MEMORY.md` や topic files は直接変更しない。公開 dream reviewer lane は `cam dream candidates` / `cam dream review` / `cam dream adopt` / `cam dream promote-prep` / `cam dream promote` / `cam dream apply-prep` に揃える。blocked な subagent candidate は先に `adopt` が必要で、durable-memory candidate は既存の reviewer/audit 経路を通る明示的な `promote` でのみ書き込まれる。instruction-like の `promote` / `promote-prep` / `apply-prep` はすべて `proposal-only` のままで、`patchPreview`、`artifactPath`、`manualWorkflow`、`applyReadiness` を含む proposal artifact を返すだけで instruction files は直接編集しない。instruction-like lane は既定の ranking を変えずに明示的な `--target-file` override も受け付け、proposal-only promote 後は `manual-apply-pending` reviewer state に入る |
...
- dream reviewer lane は `cam dream candidates` / `cam dream review` / `cam dream adopt` / `cam dream promote-prep` / `cam dream promote` / `cam dream apply-prep` に収束させている。subagent candidate は blocked で始まり、durable-memory candidate の promote は既存の reviewer/audit 経路を通じてのみ canonical memory に書き込まれる。instruction-like の `promote` / `promote-prep` / `apply-prep` は `proposal-only` のままで、`patchPreview`、`artifactPath`、`manualWorkflow`、`applyReadiness` を含む proposal artifact を返すだけで instruction files を直接書き換えない。
