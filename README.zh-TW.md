...
| `cam dream build` / `cam dream inspect` | 建立並檢視最小可用 `dream sidecar`；它會把 continuity compaction、query-time relevant refs 與待確認的 `promotionCandidates` 寫進可審計 JSON sidecar，但不會直接改 `MEMORY.md` 或 topic files；公開的 `cam dream candidates` / `cam dream review` / `cam dream promote` 仍維持顯式 reviewer lane：durable-memory candidate 只能在顯式 promote 後透過既有 reviewer/audit 寫入 canonical memory，而 instruction-like candidate 仍保持 `proposal-only` |
...
| `cam recall search` / `timeline` / `details` | 以 `search -> timeline -> details` 的 progressive disclosure 工作流檢索 durable memory；`search` 現在預設採用 `state=auto`、`limit=8`，會先查 active，未命中再回退 archived，且保持只讀 retrieval；多詞查詢現在會跨 `id/topic/summary/details` 聚合命中，而不是要求所有 term 都落在同一個欄位；JSON 現在還會額外暴露 `retrievalMode`、`finalRetrievalMode`、`retrievalFallbackReason`、`stateResolution`、`executionSummary`、`searchOrder`、`totalMatchedCount`、`returnedCount`、`globalLimitApplied`、`truncatedCount`、`resultWindow`、`globalRank`，以及 `diagnostics.checkedPaths[].returnedCount` / `droppedCount`，把 explicit-state、global sorting、fallback 與 post-limit 行為說清楚；另外也會透過 additive `querySurfacing` 給出 `suggestedDreamRefs` 與 `suggestedInstructionFiles`，但它們只做 reviewer hints，不會改動 `results[]` 或 canonical memory；其中 `finalRetrievalMode` 只是最終結果面的顯式別名 |
...
| `cam session load` / `status` | continuity reviewer surface；`status --json` / `load --json` 現在也會額外暴露 additive `resumeContext`，包括目前 goal、`suggestedDurableRefs` 與 instruction files，方便下一輪對話知道從哪裡接著做 |
...
- `cam session status --json` / `cam session load --json` 現在也會額外暴露 `resumeContext`；其中 `suggestedDurableRefs` 只做 resume hints，不會直接升級成 durable memory。
- `cam recall search --json` 現在也會額外暴露 `querySurfacing`；其中 `suggestedDreamRefs` / `suggestedInstructionFiles` 只做 query-time reviewer hints，不會直接改動 search 結果或觸發 promote。
- dream reviewer lane 正在收口為 `cam dream candidates` / `cam dream review` / `cam dream promote`；其中 durable-memory candidate 的 `promote` 會透過既有 reviewer/audit 路徑顯式寫入 canonical memory，而 instruction-like candidate 仍保持 `proposal-only`，不會直接改寫 instruction files。
