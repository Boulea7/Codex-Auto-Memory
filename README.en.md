...
| Command | Purpose |
| :-- | :-- |
| `cam run` / `cam exec` / `cam resume` | compile startup memory and launch Codex through the wrapper; wrapper startup layering stays `continuity -> instruction files -> dream refs -> top durable refs -> team/shared refs` |
...
| `cam dream build` / `cam dream inspect` | build and inspect the minimal `dream sidecar`: `build` writes the auditable JSON snapshot / audit / candidate state, including pending `promotionCandidates`, while `inspect` only reads the latest snapshot, audit, and queue metadata without implicitly rebuilding the sidecar or mutating `MEMORY.md` / topic files; the public dream reviewer lane is now `cam dream candidates` / `cam dream review` / `cam dream adopt` / `cam dream proposal` / `cam dream promote-prep` / `cam dream promote` / `cam dream apply-prep` / `cam dream verify-apply`: blocked subagent candidates must be explicitly adopted before they enter the primary review lane, durable-memory candidates only write through explicit `promote` plus the existing reviewer/audit path, and instruction-like `proposal` / `promote` / `promote-prep` / `apply-prep` all remain `proposal-only`, returning a proposal artifact with `patchPreview`, `artifactPath`, `manualWorkflow`, and `applyReadiness` instead of editing instruction files; the instruction-like lane also accepts an explicit `--target-file` override during `promote-prep` / `promote` without changing the default ranking, and after a proposal-only promote it moves into a `manual-apply-pending` reviewer state that closes through `verify-apply` |
...
| `cam session load` / `status` | inspect the continuity reviewer surface; `status --json` / `load --json` now also expose additive `resumeContext`, including the current goal, `suggestedDurableRefs`, instruction files, and read-only `suggestedTeamEntries` so resume prompts stay reviewer-visible without being promoted automatically |
...
- `cam session status --json` / `cam session load --json` now also expose additive `resumeContext`; `suggestedDurableRefs` and `suggestedTeamEntries` remain reviewer hints, not durable-memory writes.
- Public wrapper-startup wording is now aligned as `continuity -> instruction files -> dream refs -> top durable refs -> team/shared refs`.
- `cam recall search --json` now also exposes additive `querySurfacing`; `suggestedDreamRefs`, `suggestedInstructionFiles`, and `suggestedTeamEntries` are reviewer hints only and do not mutate retrieval results or trigger promote automatically.
- The dream reviewer lane is being locked down around `cam dream candidates` / `cam dream review` / `cam dream adopt` / `cam dream proposal` / `cam dream promote-prep` / `cam dream promote` / `cam dream apply-prep` / `cam dream verify-apply`; subagent candidates start blocked, durable-memory candidates only write canonical memory through explicit promote plus the existing reviewer/audit path, and instruction-like `proposal` / `promote` / `promote-prep` / `apply-prep` remain `proposal-only`, returning proposal artifacts with `patchPreview`, `artifactPath`, `manualWorkflow`, and `applyReadiness` instead of editing instruction files, while `verify-apply` is the explicit reviewer closeout step for a manual instruction edit.
- `TEAM_MEMORY.md` is now the root manifest / index-only entry for the shared team pack. Reviewer-visible team entries come only from `team-memory/*.md`, and inspect / retrieval surfaces stay read-only even when `dreamSidecarAutoBuild=true`; only wrapper startup is allowed to rebuild sidecars on demand.
...
- Release-facing verification still needs `pnpm test:dist-cli-smoke` and `pnpm test:tarball-install-smoke` to run serially, so `prepack -> rimraf dist` cannot create false negatives.
