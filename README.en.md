...
| `cam dream build` / `cam dream inspect` | build and inspect the minimal `dream sidecar`; it stores continuity compaction, query-time relevant refs, read-only `teamMemory` hints, and pending `promotionCandidates` in an auditable JSON sidecar without mutating `MEMORY.md` or topic files; the public dream reviewer lane is now `cam dream candidates` / `cam dream review` / `cam dream adopt` / `cam dream promote-prep` / `cam dream promote` / `cam dream apply-prep`: blocked subagent candidates must be explicitly adopted before they enter the primary review lane, durable-memory candidates only write through explicit `promote` plus the existing reviewer/audit path, and instruction-like `promote` / `promote-prep` / `apply-prep` all remain `proposal-only`, returning a proposal artifact with `patchPreview`, `artifactPath`, `manualWorkflow`, and `applyReadiness` instead of editing instruction files; the instruction-like lane also accepts an explicit `--target-file` override without changing the default ranking, and after a proposal-only promote it moves into a `manual-apply-pending` reviewer state |
...
- The dream reviewer lane is being locked down around `cam dream candidates` / `cam dream review` / `cam dream adopt` / `cam dream promote-prep` / `cam dream promote` / `cam dream apply-prep`; subagent candidates start blocked, durable-memory candidates only write canonical memory through explicit promote plus the existing reviewer/audit path, and instruction-like `promote` / `promote-prep` / `apply-prep` remain `proposal-only`, returning proposal artifacts with `patchPreview`, `artifactPath`, `manualWorkflow`, and `applyReadiness` instead of editing instruction files.
