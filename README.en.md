...
| `cam mcp doctor` | inspect the recommended project-scoped retrieval MCP wiring, project pinning, and hook/skill fallback assets; it now also adds a `codexStack` readiness summary plus a structured `workflowContract` for the recommended route, executable bits, shared asset version, and workflow consistency, and reports alternate global wiring separately from the recommended project-scoped path without modifying host config files |
